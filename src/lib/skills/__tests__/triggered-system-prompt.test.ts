// triggered-system-prompt.ts tests — snapshot + structural assertions.
//
...
const baseTrigger: SkillTrigger = {
  output: 'document',
...
describe('buildTriggeredSkillSystemPrompt', () => {
  it('prepends the triggered-skill preamble before the base prompt', () => {
    const result = buildTriggeredSkillSystemPrompt(
      'BASE_PROMPT',
      baseTrigger,
      'skills/my-skill',
    );
    const baseIdx = result.indexOf('BASE_PROMPT');
    const preambleIdx = result.indexOf('Triggered skill execution mode');
    expect(preambleIdx).toBeGreaterThanOrEqual(0);
    expect(baseIdx).toBeGreaterThan(preambleIdx);
  });

  it('includes the skill doc path', () => {
    const result = buildTriggeredSkillSystemPrompt(
      'BASE',
      baseTrigger,
      'skills/my-skill',
  ...
  it('includes the output type', () => {
    const result = buildTriggeredSkillSystemPrompt('BASE', baseTrigger, 's/s');
  ...
  it('shows "none" when requires_mcps is empty', () => {
    const t: SkillTrigger = { ...baseTrigger, requires_mcps: [] };
    const result = buildTriggeredSkillSystemPrompt('BASE', t, 's/s');
  ...
  it('matches snapshot', () => {
    ...
    expect(result).toMatchInlineSnapshot(`
      "## Triggered skill execution mode

      You are executing the skill defined in \`skills/my-skill\` autonomously. **There is no user present to respond to you.** Questions, clarifications, and requests for permission will not be answered — they will simply cause the run to end without producing output.
